...
def main():
    ...
    parser.add_argument("--duration", type=int, default=5, help="Duration in seconds, 2-10 (default: 5)")
    parser.add_argument("--image-url", help="Input image URL or local path for image-to-video")
    parser.add_argument("--video-url", help="Input video URL or local path for video-to-video (gen4_aleph only)")
    ...
    if args.video_url:
        if args.model != "gen4_aleph":
            print("Error: --video-url requires --model gen4_aleph", file=sys.stderr)
            sys.exit(1)
