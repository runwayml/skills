...
description: "Generate videos directly using the Runway API via runnable scripts. Supports text-to-video, image-to-video, and video-to-video with gen4.5, veo3, and more."
user-invocable: true
...
```bash
uv run scripts/generate_video.py --prompt "your description" --filename "output.mp4" [--model gen4.5] [--ratio 1280:720] [--duration 5] [--image-url "..."] [--api-key KEY]
```
...
|-------|----------|-------|------|
| `gen4.5` | Highest quality, general purpose | Text and/or Image | 12 credits/sec |
...
- "edit video", "transform video" -> `gen4_aleph`
- No preference -> `gen4.5`
...
| `--ratio` | Aspect ratio (e.g. 1280:720, 720:1280) | `1280:720` |
| `--duration` | Duration in seconds (2-10) | `5` |
| `--image-url` | Image URL or local file for image-to-video | -- |
| `--video-url` | Video URL or local file for video-to-video (gen4_aleph) | -- |
...
**Text-to-video:**
```bash
uv run scripts/generate_video.py --prompt "A serene mountain landscape at sunrise with mist" --filename "2026-04-14-mountain-sunrise.mp4"
```

**Image-to-video (animate a product photo):**
```bash
uv run scripts/generate_video.py --prompt "Camera slowly zooms out, product sparkles" --image-url "product.jpg" --filename "2026-04-14-product-reveal.mp4" --model gen4_turbo
```
...
To generate many videos at once, use `+rw-batch-campaign` which calls `batch_generate.py` with fan_out support for parallel generation.
